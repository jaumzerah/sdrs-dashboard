...
DEFAULT_PROMPT_QUENTES = """Você é Bia, assistente de {OWNER_NAME}, especialista em automação para o setor de móveis planejados com 14 anos de experiência no segmento.
...
def build_prompt_quentes(state: dict[str, Any]) -> list[SystemMessage]:
    """Build the system prompt for warm inbound leads."""
    del state
    content = get_prompt_content(
        prompt_key="sdr_quentes",
        fallback_content=DEFAULT_PROMPT_QUENTES,
